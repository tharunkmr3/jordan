...
BEGIN
  EXECUTE format(
    'CREATE POLICY %1$s_select ON %1$I FOR SELECT USING (org_id IN (SELECT auth.user_org_ids()))',
    table_name
  );
  EXECUTE format(
    'CREATE POLICY %1$s_insert ON %1$I FOR INSERT WITH CHECK (org_id IN (SELECT auth.user_org_ids()))',
    table_name
  );
  EXECUTE format(
    'CREATE POLICY %1$s_update ON %1$I FOR UPDATE USING (org_id IN (SELECT auth.user_org_ids()))',
    table_name
  );
  EXECUTE format(
